...
import { TransactionContext, TransactionFormData } from '../context/TransactionContext';
import { shortenAddress } from '../utils/shortenAddress';

const commonStyles = "min-h-[70px] sm:px-0 px-2 sm:min-w-[120px] flex justify-center items-center border-[0.5px] border-gray-400 text-sm font-light text-white";

const Input = ({ placeholder, name, type, value, handleChange }: { placeholder: string, name: string, type: string, value?: string, handleChange: Function }) => (
    <input type={type}
    ...
    return (
        <div className="flex w-full justify-center items-center">
            <div className="flex mf:flex-row flex-col items-start justify-between md:p-20 py-12 px-4">
                <div className="flex flex-1 justify-start items-start flex-col mf:mr-80">
                    ...
                    {!connectedAccount && (
                        <>
                            ...
                            <span className="text-sm text-slate-500 underline-offset-2"> (Ropsten Testnet ONLY)</span>
                        </>
                    )}
                        ...
                        {isLoading
                            ...
                            : (
                                <button
                                    ...
                                    className="text-white w-full mt-2 border-[1px] p-2 border-[#3d4f7c] hover:bg-[#3d4f7c] rounded-full cursor-pointer disabled:opacity-20 disabled:cursor-not-allowed"
                                >
                                    Send now
                                </button>
                            )}
